...
```python
canoe_inst.open(canoe_cfg=fr'D:\py_canoe\demo_cfg\demo_dev.cfg', visible=True, auto_save=False, prompt_user=False)
...
```python
canoe_inst.open(fr'D:\py_canoe\demo_cfg\demo_offline.cfg')
...
```python
canoe_inst.open(fr'D:\py_canoe\demo_cfg\demo_diag.cfg')
canoe_inst.start_measurement()
wait(1)
